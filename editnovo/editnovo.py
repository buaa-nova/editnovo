...
def train(
    ...
    with ModelRunner(config, model) as runner:
        logger.info("Training a model from %s, total_number is:%d", train_peak_path, len(train_peak_path))
        for peak_file in train_peak_path:
            logger.info("  %s", peak_file)

        logger.info("Using the following validation files:")
        logger.info("Total validation files: %d", len(validation_peak_path))
